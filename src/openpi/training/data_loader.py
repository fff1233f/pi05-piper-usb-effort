...
class VideoPlaceholderDataset(Dataset[T_co]):
    """Add tiny image placeholders after video decoding has been disabled."""

    def __init__(self, dataset: Dataset, video_keys: Sequence[str]):
        self._dataset = dataset
        self._video_keys = video_keys

    def __getitem__(self, index: SupportsIndex) -> T_co:
        item = self._dataset[index]
        placeholder = np.zeros((3, 1, 1), dtype=np.uint8)
        return {**item, **dict.fromkeys(self._video_keys, placeholder)}
...
def create_torch_dataset(
    ...
    *,
    ...
    if skip_video_decoding:
        ...
        dataset = VideoPlaceholderDataset(dataset, video_keys)

    if data_config.prompt_from_task:
        tasks = dataset_meta.tasks
        if hasattr(tasks, "reset_index"):
            task_table = tasks.reset_index()
            task_column = "task" if "task" in task_table.columns else task_table.columns[0]
            tasks = {
                int(row["task_index"]): str(row[task_column])
